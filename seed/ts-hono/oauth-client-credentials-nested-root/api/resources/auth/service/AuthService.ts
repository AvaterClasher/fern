...
export interface AuthServiceMethods {
    getToken(
        req: {
            body: SeedOauthClientCredentials.auth.GetTokenRequest;
        },
        res: {
...
export class AuthService {
    ...
    public toHono(): Hono {
        this.app.post("/token", async (c) => {
            ...
            if (request.ok) {
                try {
                    let _responseBody: SeedOauthClientCredentials.auth.TokenResponse | undefined;
                    const _cookies: {
                        name: string;
                        value: string;
                    ...
                    return c.json(
                        serializers.auth.TokenResponse.jsonOrThrow(_responseBody, { unrecognizedObjectKeys: "strip" }),
                        200,
                    );
                } catch (error) {
