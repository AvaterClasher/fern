...
export interface RootServiceMethods {
    submitFormData(
        req: {
            body: SeedApi.PostSubmitRequest;
        },
        res: {
...
export class RootService {
    ...
    public toHono(): Hono {
        this.app.post("/submit", async (c) => {
            ...
            if (request.ok) {
                try {
                    let _responseBody: SeedApi.PostSubmitResponse | undefined;
                    const _cookies: {
                        name: string;
                        value: string;
                    ...
                    return c.json(
                        serializers.PostSubmitResponse.jsonOrThrow(_responseBody, { unrecognizedObjectKeys: "strip" }),
                        200,
                    );
                } catch (error) {
