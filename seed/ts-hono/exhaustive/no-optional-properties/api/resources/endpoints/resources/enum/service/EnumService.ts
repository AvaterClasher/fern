...
export class EnumService {
    ...
    public toHono(): Hono {
        this.app.post("", async (c) => {
            const request = serializers.types.WeatherReport.parse(await c.req.json());
            if (request.ok) {
                try {
                    let _responseBody: SeedExhaustive.types.WeatherReport | undefined;
                    const _cookies: {
                        name: string;
                        value: string;
                    ...
                    return c.json(
                        serializers.types.WeatherReport.jsonOrThrow(_responseBody, { unrecognizedObjectKeys: "strip" }),
                        200,
                    );
                } catch (error) {
