...
export class UnionService {
    ...
    public toHono(): Hono {
        this.app.post("", async (c) => {
            const request = serializers.types.Animal.parse(await c.req.json());
            if (request.ok) {
                try {
                    let _responseBody: SeedExhaustive.types.Animal | undefined;
                    const _cookies: {
                        name: string;
                        value: string;
                    ...
                    return c.json(
                        serializers.types.Animal.jsonOrThrow(_responseBody, { unrecognizedObjectKeys: "strip" }),
                        200,
                    );
                } catch (error) {
