...
import { HonoContext, GeneratedHonoRegister } from "@fern-typescript/contexts";
import { PackageResolver } from "@fern-typescript/resolvers";
import { partition } from "lodash-es";
import { ts } from "ts-morph";

export declare namespace GeneratedHonoRegisterImpl {
...
export class GeneratedHonoRegisterImpl implements GeneratedHonoRegister {
    private static HONO_APP_PARAMETER_NAME = "honoApp";
    private static HONO_SERVICES_PARAMETER_NAME = "services";
    ...
    constructor({
        ...
        packageResolver
    }: GeneratedHonoRegisterImpl.Init) {
    ...
    public writeToFile(context: HonoContext): void {
        context.sourceFile.addFunction({
            ...
            parameters: [
                {
                    name: GeneratedHonoRegisterImpl.HONO_APP_PARAMETER_NAME,
                    type: getTextOfTsNode(context.externalDependencies.hono.Hono._getReferenceToType())
                },
                {
                    name: GeneratedHonoRegisterImpl.HONO_SERVICES_PARAMETER_NAME,
            ...
            statements: this.packageResolver
                ...
                .flatMap((packageId) => {
                    ...
                    let statement: ts.Statement = ts.factory.createExpressionStatement(
                        context.externalDependencies.hono.Hono.route({
                            referenceToApp: ts.factory.createIdentifier(
                                GeneratedHonoRegisterImpl.HONO_APP_PARAMETER_NAME
                            ),
                            path: ts.factory.createStringLiteral(convertHttpPathToExpressRoute(service.basePath)),
                            service: context.honoService
                                .getGeneratedHonoService(packageId)
                                .toHono(this.getReferenceToServiceArgument(packageId))
                        })
                    );
    ...
    private getReferenceToServiceArgument(
        ...
        const referenceToPackage = this.packageResolver
            ...
            .fernFilepath.packagePath.reduce<ts.Expression>(
                ...
                ts.factory.createIdentifier(GeneratedHonoRegisterImpl.HONO_SERVICES_PARAMETER_NAME)
            );
    ...
    private getPropertySignatureForService(packageId: PackageId, context: HonoContext): ts.TypeElement {
        return ts.factory.createPropertySignature(
            ...
            context.honoService
                .getReferenceToHonoService(packageId, {
